...
- `jobfeed-adapter-troubleshooting` - load before scoping or acting on job_feed adapter, seed account, or sync pipeline work under `/opt/stacks/saas-starter/job_feed`, including provider zero-or-stale jobs, short or teaser descriptions, seed or quarantine or sync errors, coverage or freshness findings, and adding a provider; it encodes the environment constraints (OOM caps, MCP timeouts) and known non-bug root causes that naive approaches trip on.
